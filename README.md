...
- **db** - PostgreSQL база данных (запускается с healthcheck: `app` не стартует, пока база не готова)
...
Скрипт `setup.sh` сам выполняет весь путь: установку зависимостей Laravel, подготовку `.env`, генерацию `APP_KEY`, миграции и выставление корректных прав доступа. Скрипт безопасно запускать повторно — он проверяет текущее состояние на каждом шаге и не перезаписывает уже настроенные файлы.
...
```
Ожидаемый ответ — `HTTP/1.1 200 OK` со стартовой страницей Laravel. 
...
### PostgreSQL долго проходит healthcheck при первом запуске

При самом первом запуске (пустой volume) инициализация Postgres может занять несколько секунд дольше обычного — `docker compose up` в это время просто ждёт, контейнер `app` стартует автоматически сразу после того, как база пройдёт healthcheck. Дополнительных действий не требуется.
